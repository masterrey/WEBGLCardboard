...
namespace TiltShift.Cardboard
{
    public class CardboardController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            PlayerPrefs.GetFloat("manualzoom", manualzoom);
        }
...
// code for URP or HDRP
        ...
        public float manualZoom=1; // The variable where the current zoom level is stored

        private Vector2[] oldTouchPositions; // positions of fingers in previous frame
        private Vector2[] newTouchPositions; // positions of fingers in current frame
        private float oldTouchDistance; // distance between fingers in previous frame
